...
class JsonNewsState extends State<JsonNews>{
            ...
            body: Center(
                child: Container(
                  ...
                  child: SingleChildScrollView(
                    child: Text.rich(
                      TextSpan(children: _getHighlighting(article.sentences, article.highlightIdx))))))
          );
        },
      ), // ...to here.
    );
  ...
  List<TextSpan> _getHighlighting(List<List<String>> article, List<int> highlight){
    final List<TextSpan> temp = [];
    var cnt = 0;
    for (var list in article) {
      for (var news in list) {
        if(highlight.indexWhere((element) => element == cnt) >= 0){
          temp.add(TextSpan(text: news.toString(), style: const TextStyle(backgroundColor: Colors.yellow,)));
        }else{
...
class JsonNews extends StatefulWidget{
  const JsonNews({Key? key}) : super(key: key);
